describe('Solar Powered Car Workshop - Future School 02 Test', () => {
  beforeEach(() => {
  ...
  it('should find Solar Powered Car Workshop (alternate time) and register successfully', () => {
    ...
    cy.get('a.group.rounded-xl.bg-white.flex.flex-col', { timeout: 15000 })
      ...
      .then($elements => {
        ...
        for (let i = 0; i < $elements.length; i++) {
          ...
          // Look for workshop with August 02, 2025 date or any Solar Car workshop
          if ((text.includes('Solar Powered Car Workshop') && 
               text.includes('Saturday, August 23, 2025')) &&
        ...
        // Strategy 2: Look for any Solar Car Workshop
        if (!matchedElement) {
          for (let i = 0; i < $elements.length; i++) {
            const el = $elements[i];
            const text = (el.innerText || '');
        ...
        // Strategy 3: Look for Future School workshop
        if (!matchedElement) {
          for (let i = 0; i < $elements.length; i++) {
            const el = $elements[i];
            const text = (el.innerText || '');
            ...
            if (text.includes('Future School') && 
                (text.includes('$9') || text.includes('09:00 AM') || text.includes('CST'))) {
    ...
    // Handle time slot selection (if exists) - Try different time slot for 02 version
    cy.get('body').then($body => {
      // Try to select a different time slot than the first test
      if ($body.find('span:contains("09:00 AM")').length > 0) {
      ...
      } else if ($body.find('span:contains("PM")').length > 0) {
        // Select any PM slot
        cy.get('span:contains("PM")').first().click({ force: true });
        cy.log('✅ Selected PM time slot');
      } else if ($body.find('input[type="radio"]').length > 0) {
        // Select second available time slot if exists
        if ($body.find('input[type="radio"]').length > 1) {
          cy.get('input[type="radio"]').eq(1).check();
          cy.log('✅ Selected second available time slot');
        } else {
    ...
    cy.get('body').then($body => {
      ...
      if ($body.find('input[placeholder*="Address"]').length > 0) {
        cy.get('input[placeholder*="Address"]').type('456 Oak Street, Houston, TX');
      }
      
      // City field
      if ($body.find('input[placeholder*="City"]').length > 0) {
        cy.get('input[placeholder*="City"]').type('Houston');
      }
      ...
      if ($body.find('input[placeholder*="Zip"]').length > 0) {
        cy.get('input[placeholder*="Zip"]').type('77001');
      } else if ($body.find('input[placeholder*="Pincode"]').length > 0) {
        cy.get('input[placeholder*="Pincode"]').type('77001');
      }
    });
